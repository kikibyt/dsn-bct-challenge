...
def evaluate_task_a(num_samples: int = 10):
    ...
    - Compare simulated review to actual review using ROUGE + BERTScore
    ...
    for user_id, user_review_list in test_users:
        ...
        try:
            ...
            references.append(test_review["text"])
            hypotheses.append(result["simulated_review"])
   ...
   # Calculate BERTScore
    print("\nCalculating BERTScore (this takes a moment)...")
    try:
        if references and hypotheses:
            P, R, F1 = bert_score(hypotheses, references, lang="en", 
                                   verbose=False, model_type="distilbert-base-uncased")
            bert_f1 = float(F1.mean())
        else:
            bert_f1 = 0.0
    except Exception as e:
        print(f"BERTScore error: {e}")
        print("Falling back to cosine similarity approximation...")
        bert_f1 = 0.0
    ...
    print(f"BERTScore:  {results['bertscore_f1']}")
...
def evaluate_task_b(num_samples: int = 10):
    ...
    for user_id, user_review_list in test_users:
        # History = first N-2, ground truth = last 2
        ...
        try:
            ...
            # Match recommended names to business IDs
...
if __name__ == "__main__":
    ...
    print(f"Task A — BERTScore:  {task_a['bertscore_f1']}")
